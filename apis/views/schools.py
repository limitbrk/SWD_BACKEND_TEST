...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if not (isinstance(student_first_name,str)
            and isinstance(student_last_name,str)
            and isinstance(subjects_title,str)
            and isinstance(score,int)
            ):
            return Response({"error":"type not match"},status=status.HTTP_400_BAD_REQUEST)

        student = Personnel.objects.filter(first_name=student_first_name,last_name=student_last_name).first()
        subject = Subjects.objects.filter(title=subjects_title).first()

        if not student or not subject: 
            return Response({"error":"not found"},status=status.HTTP_400_BAD_REQUEST)
            
        subject_score = StudentSubjectsScore.objects.filter(student_id=student.pk,subjects_id=subject.pk).first()
        if subject_score:
            subject_score.score = score
            subject_score.save()
        else:
            credit_id = None
            for value in credits_mapping:
                if subject.pk == value['subject_id']:
                    credit_id = value['credit_id']
            credit_score = None
            for value in credits_context:
                if credit_id == value['id']:
                    credit_score = value['credit']
            
            if not credit_id and not credit_score:
                return Response({"error":"invalid credit"},status=status.HTTP_400_BAD_REQUEST)
            subject_score = StudentSubjectsScore.objects.create(credit=credit_score, score=score, student_id=student.pk, subjects_id=subject.pk)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        student = Personnel.objects.filter(pk=student_id).first()

        subject_scores = StudentSubjectsScore.objects.filter(student_id=student.pk)
        subject_detail = []
        sum_score = 0
        all_credit = 0
        for score in subject_scores:
            subject_detail.append({
                ...
                "credit": score.credit,
                "score": score.score,
                "subject": getGrade(score.score),
            })
            sum_score += score.credit*score.score
            all_credit += score.credit
        ...
        rs = {
            ...
            "grade_point_average": getGrade(sum_score/all_credit),
        }
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        classes_id = Classes.objects.filter(school_id=school.pk).values_list("id",flat=True)

        persons = Personnel.objects.filter(school_class_id__in=classes_id)
        persons_detail = []
        ...
        return Response(persons_detail, status=status.HTTP_200_OK)
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for school in schools:
            ...
            classes = Classes.objects.filter(school_id=school.pk)
            for clazz in classes:
                class_result={}
                student_result=[]
                students = Personnel.objects.filter(school_class_id=clazz.pk,personnel_type__in=[1,2])
                for student in students:
                    student_result.append({
                        role_map[student.personnel_type]: " ".join([student.first_name,student.last_name])
                    })
                teacher = Personnel.objects.filter(school_class_id=clazz.pk,personnel_type=0).first()
